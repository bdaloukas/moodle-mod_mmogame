...
class renderer extends plugin_renderer_base {
    ...
    /**
     * Returns the same as {@see mmogame_num_attempt_summary()} but wrapped in a link to the quiz reports.
     *
     * @param stdClass $mmogame the mmogame object. Only $quiz->id is used at the moment.
     ...
     */
    public function mmogame_attempt_summary_link_to_reports(stdClass $mmogame, stdClass $cm, context $context,
            bool $returnzero = false, int $currentgroup = 0) {
        global $CFG;
        $summary = mmogame_num_attempt_summary($mmogame, $cm, $returnzero, $currentgroup);
        if (!$summary) {
            return '';
        }
        ...
        $url = new moodle_url('/mod/mmogame/report.php', [
                'id' => $cm->id, 'mode' => mmogame_report_default_report($context)]);
